...
import { BaseTeam, BaseTeamsListProps } from './types';

export function BaseTeamsList<T extends BaseTeam>({
  teams,
  selectedTeam,
  onSelectTeam,
  onEditTeam,
  renderExtraStats // Nova prop opcional para customizar estatísticas adicionais
}: BaseTeamsListProps<T> & {
  renderExtraStats?: (team: T) => React.ReactNode;
}) {
  if (teams.length === 0) {
    return (
        ...
        >
          ...
          {team.players.length > 0 && (
            <div className="mt-4 flex items-center justify-between text-sm">
              ...
              {/* Renderiza estatísticas extras personalizadas se fornecidas */}
              {renderExtraStats ? renderExtraStats(team) : (
                <div className="flex items-center space-x-2">
