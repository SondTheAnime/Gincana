  ...
  const checkInscricoesStatus = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('inscricoes_times_abertas')
        .single()

      if (error) throw error

      if (data) {
        setInscricoesAbertas(data.inscricoes_times_abertas)
      }
    } catch (error) {
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 text-center">
            <div className="flex flex-col items-center justify-center space-y-4">
              ...
              <p className="text-gray-500 dark:text-gray-400 max-w-md">
                As inscrições de times estão temporariamente fechadas. Por favor, tente novamente mais tarde.
              </p>
            </div>
          </div>
        </div>
